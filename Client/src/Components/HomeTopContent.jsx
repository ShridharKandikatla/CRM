import { FiBell } from "react-icons/fi";
import { FaPlus } from "react-icons/fa";
// import { BsPersonCircle } from "react-icons/bs";
import HomeContent from "./HomeContent"
import HomePageButton from "./HomePageButton";
import Profile from "./Profile";

const HomeTopContent = () => {
    return (
        <>
            <div className="container mx-auto px-4 sm:px-8 bg-gray-200">
                {/* <div className="py-8"> */}
                <div className=" my-8 ">
                    <div className="my-2">


                        <div className="flex justify-between mb-1 sm:mb-0">

                            <div className="mt-3 flex w-28 sm:flex-row sm:w-64 lg:w-fit">
                                <div>
                                    <div className="relative">
                                        <select className="appearance-none h-full rounded-l border block appearance-none w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                            <option>Basic Search</option>
                                            <option>10</option>
                                            <option>20</option>
                                        </select>
                                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                            {/* <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                                <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                                            </svg> */}
                                        </div>
                                    </div>
                                </div>


                                <div className="block relative">
                                    <span className="h-full absolute inset-y-0 left-0 flex items-center pl-2">
                                        <svg viewBox="0 0 24 24" className="fill-current text-gray-500">
                                            <path d="M10 4a6 6 0 100 12 6 6 0 000-12zm-8 6a8 8 0 1114.32 4.906l5.387 5.387a1 1 0 01-1.414 1.414l-5.387-5.387A8 8 0 012 10z">
                                            </path>
                                        </svg>
                                    </span>
                                    <input placeholder="Search" className="appearance-none rounded-r rounded-l sm:rounded-l-none border border-gray-400 border-b block pl-8 pr-6 py-2 w-full bg-white text-sm placeholder-gray-400 text-gray-700 focus:bg-white focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none" />
                                </div>


                                <div className="pl-3 m-2">
                                    <FiBell className="h-5 w-5" />
                                </div>


                                <div className=" m-2">
                                    <FaPlus className="h-5 w-5" />
                                </div>


                            </div>
                            <div>

                                <div className=" m-2 pl-2">
                                    {/* <BsPersonCircle className="h-10 w-10" /> */}
                                    <Profile />
                                </div>
                            </div>

                        </div>


                    </div>
                </div>
                <HomePageButton />
                <HomeContent />
                {/* </div> */}
            </div>
        </>
    )
}
export default HomeTopContent